...
class RealFunctionBase{
	...
	private:
		mutable std::shared_ptr<const RealFunctionBase> derivative = nullptr;
		
		virtual const RealFunctionBase *calculateDerivative() const = 0;
	...
	public:
		...
		/**
		 * @brief Dynamic vSumFunction::SumFunctionirtual clone constructor
		 * @return RealFunctionBase* copy of the object
		 */

		std::shared_ptr<const RealFunctionBase> getDerivative() const;

		virtual RealFunctionBase *clone() const = 0;
